...
//29.06.2021 YN was flash_seg   =0xC000
...
//29.06.2021 YN seg_tmp=0xC000;
  seg_tmp=0xD000;

 //vc= get_vdate((unsigned int)0xC000); //29.06.2021 YN
 vd= get_vdate((unsigned int)0xD000);
 ve= get_vdate((unsigned int)0xE000);

 if(/*(vc==0)&&*/(vd==0)&&(ve==0))
  {  // все секторы очищены, начало записи //29.06.2021 YN
    //29.06.2021 YN
    flash_seg=0xD000;
    flash_offset=0;
    return;
...
*/
  i=0;
  /*if(vc>i)
   {i=vc; seg_tmp=0xC000;}*/ //29.06.2021 YN
  if(vd>i)
   {i=vd; seg_tmp=0xD000;}
  if(ve>i)
   {i=ve; seg_tmp=0xE000;}
...
unsigned int seg_tmp;
 ...
 else if(seg==0xE000) seg_tmp=0xD000; //29.06.2021 YN was seg_tmp=0xC000;
 //else seg_tmp=0xC000; //29.06.2021 YN
...
unsigned int seg_tmp;
 ...
 else if(seg==0xE000) seg_tmp=0xD000; //29.06.2021 YN was c000
 //else seg_tmp=0xC000; seg_tmp=0xD000; //29.06.2021 YN
 return seg_tmp;
}
//-------------------
...
unsigned int seg_tmp;
 ...
 if(seg==0xD000) seg_tmp=0xE000; //29.06.2021 YN was seg_tmp=0xC000;
 else if(seg==0xE000) seg_tmp=0xD000;
 //else if(seg==0xC000) seg_tmp=0xE000; //29.06.2021 YN
 //else seg_tmp=0xC000; //29.06.2021 YN
...
//------------------------------------------
unsigned int d_fl_seg=0xD000, d_fl_offs=0; //29.06.2021 YN was 0xC000
